...
<div class="buttons show">			
		<s:if test="auth.isAdmin">
			<s:if test="showData.depth < 10">			
				<a href="write?id=${param.id}&board_no=${param.board_no}&depth=${showData.depth+1}&re=re&subject=${showData.subject}&amp;contents=${param.contents}"  class="artn-button board">답글</a>
			</s:if>
			<a class="artn-button board" href="edit?id=${showData.id }&amp;board_no=${param.board_no}&amp;contents=${param.contents}">수정</a>
			<a href="delete?id=${param.id}&amp;board_no=${param.board_no}&amp;contents=${param.contents}" class="artn-button board">삭제</a>
		</s:if>	
		<s:else>
			<a class="artn-button board" href="edit?id=${showData.id }&amp;board_no=${param.board_no}&amp;contents=${param.contents}" data-rule="confirmButton" data-dialog="#dialog">수정</a>
			<a href="delete?id=${param.id}&amp;board_no=${param.board_no}&amp;contents=${param.contents}" data-rule="confirmButton" class="artn-button board" data-dialog="#dialog">삭제</a>	
		</s:else>			
